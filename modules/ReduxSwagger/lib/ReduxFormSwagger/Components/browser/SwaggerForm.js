...
import { getSummaryForOperationById } from '../../data/utilities';
...
class SwaggerForm extends React.Component {
	...
	componentDidMount() {
		isMounted = true;
		this.setState({ summary: getSummaryForOperationById(this.props.id) });
	}

	render() {
		if (typeof this.props.children === 'function') {
			return <form onSubmit={this.props.handleSubmit}>{this.props.children(this.props)}</form>;
		} else {
...
function mapStateToProps(state, props) {
	return {
		...
		onSubmit: (payload, dispatch, formProps) => {
			...
			return apiCreator(props.id, { body: newPayload })
				.then(apiPayload => {
