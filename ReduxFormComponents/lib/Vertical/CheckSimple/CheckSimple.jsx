...
class CheckSimple extends React.Component {
  ...
  componentWillMount() {
    const {
      input, name, autoFocus, className, label, placeholder, type, meta: {
      ...
      <FormElementWrapper {...props} _id="fe-check-simple">
        <div className="form-input">
          <div className="checkbox checkbox-inline">
            <input defaultChecked={isChecked} {...input} className="checkbox-input" type="checkbox" id={htmlForId}/>
            <label
              className="checkbox-label"
              htmlFor={htmlForId}>{label}</label>
          </div>
        </div>
        {helperText &&
